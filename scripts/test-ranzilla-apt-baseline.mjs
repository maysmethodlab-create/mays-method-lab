#!/usr/bin/env node
/**
 * APT baseline run — generate Sean McGuire's letter for Sam Ranzilla
 * (Executive Professor, Accounting) via the platform pipeline. Used as
 * the validation case for the Executive Professor / Professor of Practice
 * subtype (role-category id `apt-practice`). Ranzilla's packet has no
 * CV/F180; the letter itself stands in as the source.
 */
...
const OUT_DIR = path.join(APP_DIR, 'test-output', 'ranzilla-apt-baseline');

// No CV/F180 in inventory — fall back to the letter text itself.
const FILES = ['Ranzilla_Annual_Review_2024.txt'];

const RATINGS = {
  // From Sean's actual 2024 letter for Ranzilla (teaching and service excellent).
...
// the auditing-class teaching, the unusual research-workshop participation,
// and the Aggie Accounting Association advising. Forward-look: continued
// research-workshop engagement and AAA advising.
const NOTES = `Outstanding auditing teacher. Eminently qualified, very positive student evaluations across all auditing courses (ACCT 463 / ACCT 638). Actively mentors students in class and on professional careers. Thank you Sam for investing in our students.

Unique among APT faculty: Sam attends our research workshops actively and provides meaningful one-on-one feedback to research faculty on their work. Continue this engagement; very valuable to the colleagues he supports.

Service: faculty advisor for the Aggie Accounting Association (AAA). The AAA is a critical recruiting gateway for our accounting major and Sam's mentorship of those students has a real impact. Thank you for that work.
...
(async () => {
  ...
  await login();
  console.log('\n=== Sam Ranzilla (Sean McGuire / Accounting / APT executive professor) ===');
  ...
  const setup = {
    writerId: 'mcguire',
    evaluationYear: 2024,
    recipientName: 'Sam Ranzilla',
    recipientTitle: 'Executive Professor',
